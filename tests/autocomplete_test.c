...
// Helper to reset buffer logic if needed, but perform_autocomplete is stateless regarding previous calls 
// except for the buffer content itself.
...
void test_autocomplete_file(void)
{
    ...
    // Tests might run from root (based on Makefile 'test' target: ./tests/main_test? No, build/main_test)
    // The CWD when running "make test" depends on how it is invoked.
    // Usually "make test" runs the executable.
    ...
    // Clean up first to ensure we don't leave junk even if assert fails (OS cleans up eventually/git ignore)
